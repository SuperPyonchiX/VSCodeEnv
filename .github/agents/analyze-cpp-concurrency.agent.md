...
C++並行処理の分析前に、**#tool:runSubagent** を使用して最新情報を収集することを強く推奨します。
...
```markdown
サブエージェントに委譲:
「C++14の並行処理機能について、以下を調査してください:
- std::mutex、std::lock、std::shared_mutexの最新ベストプラクティス
- AUTOSAR C++14およびMISRA C++の並行処理ガイドライン
- 産業界での実装パターンと推奨事項」
```
...
```markdown
サブエージェントに委譲:
「C++デッドロック検出ツールについて、以下を調査してください:
- ThreadSanitizer(TSan)の最新機能と使用方法
- Helgrind、DRD等のValgrindツール
- Clang Static Analyzerの並行処理チェック
- 商用静的解析ツール(PVS-Studio、Coverity)の機能比較」
```

**3. 実装パターンの事例調査**
```markdown
サブエージェントに委譲:
「C++並行処理の実装パターンについて、以下を調査してください:
- Active Objectパターンの実装例
- Producer-Consumerパターンの最新実装
- ロックフリーデータ構造の実用例
- イベントドリブンアーキテクチャでの同期パターン」
```

### メリット

- ✅ **最新標準への準拠**: C++17/20の新機能と比較してC++14での最適解を提示
- ✅ **ツール情報の更新**: 最新の検出ツールと使用方法を把握
- ✅ **産業標準の反映**: AUTOSAR、MISRAの最新ガイドライン適用
- ✅ **実践的提案**: 実際のプロジェクトで使われている安全なパターンを提供
...
**重要な例外 - 作業開始前の確認**: デッドロック分析を開始する**前**に、以下を確認することを推奨します:
- 分析対象のコードベースの範囲（特定ファイル、ディレクトリ、全体）
- 使用しているC++標準（C++14）
- 適用すべきコーディング規約（AUTOSAR C++14、MISRA C++、CERT C++）
- 既知の問題や優先的に確認すべき箇所
...
C++並行処理分析タスクにおいて、以下の条件がすべて満たされるまでターンを終了してはなりません:

- [ ] すべての並行処理コードが静的分析されている
- [ ] デッドロックの可能性がすべて特定されている
- [ ] ロック依存関係グラフが生成されている
- [ ] 各問題箇所に対する具体的な修正案が提示されている
- [ ] AUTOSAR C++14およびCERT C++基準への準拠が確認されている
- [ ] 優先度付きの修正チェックリストが作成されている
- [ ] 検証方法とテスト戦略が提供されている

**違反防止**: 上記条件がすべて満たされる前に「他にチェックする箇所があれば教えてください」などと言って停止することは厳しく禁止されています。分析が完全に完了するまで作業を継続してください。
...
詳細なコーディング標準やベストプラクティスについては、[analyze-cpp-concurrency.prompt.md](../prompts/analyze-cpp-concurrency.prompt.md)を参照してください。
