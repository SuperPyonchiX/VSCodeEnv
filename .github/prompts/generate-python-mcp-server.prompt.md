...
**必須バージョン**: Python 3.10以上、mcp[cli] 1.2.0以上

1. **プロジェクト構造**: uvを使用して適切な構造で新しいPythonプロジェクトを作成
2. **依存関係**: uvでmcp[cli]パッケージ（1.2.0以上）を含める
...
- **outputSchema**: ツールに出力JSON Schemaを定義
  - Pydanticモデルからスキーマを自動生成
  - 構造化された結果を返し、TextContentにもJSONを含める
- Pydanticモデルによる構造化出力
- UI表示用のアイコン
- Imageクラスによる画像処理
- より良いUXのための補完サポート
...
### 🚨 重要: STDIOログ出力に関する警告
- **STDIOサーバーで絶対に禁止**: `print()` ステートメントまたはstdoutへの出力
  - 理由: stdoutへの出力はJSON-RPCメッセージを破壊します
- **正しいログ記録方法**:
  - Pythonの`logging`モジュールを使用（自動的にstderrに出力）
  - Contextパラメータ: `await ctx.info()`, `await ctx.error()`
...
### 必須対策（MUST）
- **入力検証**: すべてのツール入力を検証（Pydantic/TypedDictでスキーマ定義）
